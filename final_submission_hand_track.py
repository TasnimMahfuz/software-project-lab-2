...
def main():
    ...
    while cap.isOpened():
        ...
        if results_pose.pose_landmarks:
            ...
            # Initialize both_hands_in_red_zone
            both_hands_in_red_zone = False

            # Check if both hands are in the red zone
            if len(hands_positions) >= 2:
                both_hands_in_red_zone = all(is_in_zone(hand_position, right_zone) for hand_position in hands_positions[:2])
                if both_hands_in_red_zone and not hands_in_red_zone:
                    count += 1
