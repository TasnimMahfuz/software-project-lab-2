...
def log_activity(employee_id, activity_type, product_count=0):
    ...
    cursor.execute('''
    INSERT INTO ActivityLog (employee_id, timestamp, activity_type, product_count)
    VALUES (?, ?, ?, ?)
    ''', (employee_id, timestamp, activity_type, product_count))

    conn.commit()
    conn.close()
    print(f"Logged {activity_type} for {employee_id} with product count {product_count} at {timestamp}")
...
class HandTrackingApp:
    ...
    def create_report(self, emp_id, emp_name):
        ...
        cursor.execute('''
        SELECT activity_type, SUM(product_count), SUM(CASE WHEN activity_type = 'working' THEN 1 ELSE 0 END) * 2, 
        SUM(CASE WHEN activity_type = 'idle' THEN 1 ELSE 0 END) * 2
        FROM ActivityLog
        ...
        for row in data:
            if row[0] == 'working':
                total_product_count += row[1]
                total_working_time += row[2]
            elif row[0] == 'idle':
                total_idle_time += row[3]

        report = f"Report for {emp_name} (ID: {emp_id}) on {current_date}:\n\n"
        report += f"Total Product Count: {total_product_count}\n"
        report += f"Total Working Time: {total_working_time} seconds\n"
        report += f"Total Idle Time: {total_idle_time} seconds\n"
    ...
    def update(self):
        ...
        if results_pose.pose_landmarks:
            ...
            if len(hands_positions) >= 1:
                any_hand_in_red_zone = any(self.is_in_zone(hand_position, right_zone) for hand_position in hands_positions)
                if any_hand_in_red_zone and not self.hands_in_red_zone:
                    self.count += 1
                    self.hands_in_red_zone = True
                    log_activity(self.emp_id, 'working', self.count)  # Log working activity
                    print(f"Working: Product Count = {self.count}")
